...
class ChatDeepSeek(BaseChatModel):
	...
	async def ainvoke(
		...
		**kwargs: Any,
		...
		# ② Function Calling path (only when tools are explicitly provided)
		if tools:
			try:
				call_tools = tools
				tool_choice = None
		...
		# ③ JSON Output path (official response_format) - preferred for structured output
		...
		# ④ Function Calling fallback for output_format (if JSON Output fails or not supported)
		if output_format is not None and hasattr(output_format, 'model_json_schema'):
			try:
