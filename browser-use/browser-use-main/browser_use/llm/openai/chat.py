                ...
                try:
                        ...
                        if output_format is None:
                                ...
                                if self.add_schema_to_system_prompt and openai_messages and openai_messages[0]['role'] == 'system':
                                        print(f"\n[CHAT.PY] 进入 add_schema_to_system_prompt=True 分支")
                                        schema_text = f'\n<json_schema>\n{response_format}\n</json_schema>'
                                ...
                                if self.dont_force_structured_output:
                                        # 🎯 先确认执行到这个分支
                                        print(f"\n[CHAT.PY] 进入 dont_force_structured_output=True 分支")
                                        print(f"[CHAT.PY] model={self.model}, base_url={self.base_url}")
                                        print(f"[CHAT.PY] openai_messages={openai_messages}")
                                        # 使用原生的 JSON 模式
                                        response = await self.get_client().chat.completions.create(
                                                model=self.model,
                                                messages=openai_messages,
                                                response_format={"type": "json_object"},  # 添加这行关键代码
                                                **model_params,
                                        )
                                        ...
                                        # Return structured response with strict JSON schema
                                        print(f"\n[CHAT.PY] 进入 dont_force_structured_output=False 分支")
                                        response = await self.get_client().chat.completions.create(
                                                model=self.model,
                                                messages=openai_messages,
