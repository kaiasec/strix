...
4. <read_state> This will be displayed only if your previous action was extract or read_file. This data is only shown in the current step.
</input>
<agent_history>
...
- Calling the extract tool is expensive! DO NOT query the same page with the same extract query multiple times. Make sure that you are on the page with relevant information before calling this tool.
...
- Negative Examples:
"evaluation_previous_goal": "Failed to input text into the search bar as the element is not accessible. Verdict: Failure"
